import '../credit_card_and_debit_screen/widgets/cards_item_widget.dart';import 'bloc/credit_card_and_debit_bloc.dart';import 'models/cards_item_model.dart';import 'models/credit_card_and_debit_model.dart';import 'package:ecommerce_shop/core/app_export.dart';import 'package:ecommerce_shop/widgets/app_bar/appbar_leading_image.dart';import 'package:ecommerce_shop/widgets/app_bar/appbar_subtitle.dart';import 'package:ecommerce_shop/widgets/app_bar/custom_app_bar.dart';import 'package:ecommerce_shop/widgets/custom_elevated_button.dart';import 'package:flutter/material.dart';class CreditCardAndDebitScreen extends StatelessWidget {const CreditCardAndDebitScreen({Key? key}) : super(key: key);

static Widget builder(BuildContext context) { return BlocProvider<CreditCardAndDebitBloc>(create: (context) => CreditCardAndDebitBloc(CreditCardAndDebitState(creditCardAndDebitModelObj: CreditCardAndDebitModel()))..add(CreditCardAndDebitInitialEvent()), child: CreditCardAndDebitScreen()); } 
...
/// Section Widget
Widget _buildCards(BuildContext context) { return Padding(padding: EdgeInsets.symmetric(horizontal: 16.h), child: BlocSelector<CreditCardAndDebitBloc, CreditCardAndDebitState, CreditCardAndDebitModel?>(selector: (state) => state.creditCardAndDebitModelObj, builder: (context, creditCardAndDebitModelObj) {return ListView.separated(physics: NeverScrollableScrollPhysics(), shrinkWrap: true, separatorBuilder: (context, index) {return SizedBox(height: 14.v);}, itemCount: creditCardAndDebitModelObj?.cardsItemList.length ?? 0, itemBuilder: (context, index) {CardsItemModel model = creditCardAndDebitModelObj?.cardsItemList[index] ?? CardsItemModel(); return CardsItemWidget(model, onTapCreditCardItem: () {onTapCreditCardItem(context);});});})); } 
/// Section Widget
